...
export const setResources =
  ...
  (c: Cluster) => {
    ...
    const containers = [
      ..._.flatten(
        c.Deployments.map(([, d]) => d.spec?.template.spec?.containers)
      ),
      ..._.flatten(
...
export const setReplicas =
  ...
  (c: Cluster) => {
    ...
    c.StatefulSets.filter(
      ([, statefulSet]) =>
        _.includes(deploymentAndStatefulSetNames, statefulSet.metadata?.name) &&
        statefulSet.spec
    ).forEach(([, statefulSet]) => (statefulSet.spec!.replicas = replicas));
    return Promise.resolve();
  };

export const setNodeSelector =
  (
  ...
  (c: Cluster) => {
    c.Deployments.filter(
      ...
      ([, deployment]) =>
        (deployment.spec!.template.spec!.nodeSelector = _.merge(
          {},
    ...
    c.StatefulSets.filter(
      ...
      ([, statefulSet]) =>
        (statefulSet.spec!.template.spec!.nodeSelector = _.merge(
          {},
...
export const setAffinity =
  ...
  (c: Cluster) => {
    c.Deployments.filter(
      ...
      ([, deployment]) => (deployment.spec!.template.spec!.affinity = affinity)
    );
    c.StatefulSets.filter(
      ([, statefulSet]) =>
        _.includes(deploymentAndStatefulSetNames, statefulSet.metadata?.name) &&
