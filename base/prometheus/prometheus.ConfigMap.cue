...
configMap: prometheus: {
	...
	data: {
		...
		"alert_rules.yml": yaml656e63.Marshal(_cue_alert_rules_yml)
		let _cue_alert_rules_yml = {
			groups: [{
				name: "alert.rules"
				rules: [{
					alert: "PodsMissing"
					...
					expr:  "histogram_quantile(0.9, sum by(le) (rate(src_http_request_duration_seconds_bucket{job=\"sourcegraph-frontend\",route!=\"raw\"}[10m]))) > 20"
					...
					expr:  "http_response_size_bytes{handler=\"prometheus\",job!=\"kubernetes-apiservers\",job!=\"kubernetes-nodes\",quantile=\"0.5\"} > 20000"
		...
		"extra_rules.yml": ""
		"node_rules.yml":  yaml656e63.Marshal(_cue_node_rules_yml)
		let _cue_node_rules_yml = {
			groups: [{
				name: "nodes.rules"
				rules: [{
					record: "node:container_cpu_usage_seconds_total:ratio_rate5m"
					expr:   "sum by(instance) (rate(container_cpu_usage_seconds_total{kubernetes_pod_name=\"\"}[5m])) / max by(instance) (machine_cpu_cores)"
				}, {
		...
		"sourcegraph_rules.yml": yaml656e63.Marshal(_cue_sourcegraph_rules_yml)
		let _cue_sourcegraph_rules_yml = {
			groups: [{
