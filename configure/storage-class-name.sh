...
if [ -n "$STORAGE_CLASS_NAME" ]; then
    find $BASEDIR -name "*PersistentVolumeClaim.yaml" -exec sh -c "cat {} | yj | jq '.spec.storageClassName = \"$STORAGE_CLASS_NAME\"' | jy -o {}" \;
    echo "> Using configured storage class: $STORAGE_CLASS_NAME"
else
    find $BASEDIR -name "*PersistentVolumeClaim.yaml" -exec sh -c "cat {} | yj | jq 'del(.spec.storageClassName)' | jy -o {}" \;
