...
find $BASE -name 'gitserver-*.Deployment.yaml' -exec sh -c "cat {} | yj | jq '(.spec.template.spec.containers[].volumeMounts | select(length > 0)) |= del(.[]? | select(.name == \"ssh\"))' | jy -o {}" \;
find $BASE -name 'gitserver-*.Deployment.yaml' -exec sh -c "cat {} | yj | jq '.spec.template.spec.volumes |= del(.[] | select(.name == \"ssh\"))' | jy -o {}" \;
...
if [ -n "$GITSERVER_SSH_PRIVATE_KEY_PATH" ] && [ -n "$GITSERVER_SSH_KNOWN_HOSTS_PATH" ]; then
    ...
    cat > $BASE/gitserver/gitserver-ssh.Secret.yaml <<EOM
apiVersion: v1
data:
  id_rsa: "$PRIVATE_KEY"
...
EOM
    ...
    # find $BASE -name 'gitserver-*.Deployment.yaml' -exec sh -c "cat {} | yj | jq '(.spec.template.spec.containers[] | select(.name | startswith(\"gitserver-\")).volumeMounts) += [{mountPath: \"/root/.ssh\", name: \"ssh\"}]' | jy -o {}" \;
    # Instead, just attach the .ssh volume to all containers running in gitserver.
    find $BASE -name 'gitserver-*.Deployment.yaml' -exec sh -c "cat {} | yj | jq '.spec.template.spec.containers[].volumeMounts += [{mountPath: \"/root/.ssh\", name: \"ssh\"}]' | jy -o {}" \;
    find $BASE -name 'gitserver-*.Deployment.yaml' -exec sh -c "cat {} | yj | jq '.spec.template.spec.volumes += [{name: \"ssh\", secret: {defaultMode: 384, secretName:\"gitserver-ssh\"}}]' | jy -o {}" \;
    echo "> SSH configured"
else
    rm -f $BASE/gitserver/gitserver-ssh.Secret.yaml
